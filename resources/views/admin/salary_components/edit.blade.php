<x-admin-layout>
    <x-slot name="header">
        <h2 class="text-2xl font-bold text-gray-800 tracking-tight">Edit Salary Component</h2>
    </x-slot>

    <div class="card elevation-2 border-0 rounded-lg max-w-2xl">
        <div class="card-header bg-white border-b border-gray-100 py-4">
            <h3 class="card-title text-gray-700 font-semibold">Component Details</h3>
        </div>
        <form action="{{ route('salary-components.update', $salary_component) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="card-body p-6">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="nama_component" id="nama_component" value="{{ old('nama_component', $salary_component->nama_component) }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-adminlte-primary focus:ring-adminlte-primary sm:text-sm transition-colors" required>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <select name="id_type_component" id="id_type_component" class="w-full rounded-md border-gray-300 shadow-sm focus:border-adminlte-primary focus:ring-adminlte-primary sm:text-sm transition-colors" required>
                        ...
                        @foreach($types as $type)
                            <option value="{{ $type->id_type_component }}" {{ old('id_type_component', $salary_component->id_type_component) == $type->id_type_component ? 'selected' : '' }}>
            ...
            <div class="card-footer bg-gray-50/50 py-3 px-6 flex justify-end space-x-3 rounded-b-lg">
                ...
                <button type="submit" class="btn btn-primary btn-sm px-4 py-2 rounded-md shadow-sm font-medium transition-colors hover:bg-adminlte-primary-dark">
                    <i class="fas fa-save mr-1"></i> Update Component
                </button>
            </div>
        </form>
    </div>
