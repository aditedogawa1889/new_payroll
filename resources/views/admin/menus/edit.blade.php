<x-admin-layout>
            ...
            <form action="{{ route('menus.update', $menu->id_menu) }}" method="POST" class="p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="nama_menu" value="{{ old('nama_menu', $menu->nama_menu) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-adminlte-primary focus:border-adminlte-primary" required>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="parent_id" id="parent_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-adminlte-primary focus:border-adminlte-primary" onchange="toggleUrlField()">
                            ...
                            @foreach($parents as $parent)
                                <option value="{{ $parent->id_menu }}" {{ (old('parent_id', $menu->parent_id) == $parent->id_menu) ? 'selected' : '' }}>{{ $parent->nama_menu }}</option>
                            @endforeach
                        </select>
                    ...
                    <div id="url_menu_field">
                        ...
                        <input type="text" name="url_menu" id="url_menu_input" value="{{ old('url_menu', $menu->url_menu) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-adminlte-primary focus:border-adminlte-primary" placeholder="contoh: users.index">
                    ...
                    <div>
                        ...
                        <input type="text" name="icon_menu" value="{{ old('icon_menu', $menu->icon_menu) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-adminlte-primary focus:border-adminlte-primary" placeholder="fas fa-star">
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700">Order <span class="text-red-500">*</span></label>
                        <input type="number" name="order_menu" value="{{ old('order_menu', $menu->order_menu) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-adminlte-primary focus:border-adminlte-primary" required>
                    </div>
    ...
    <script>
        function toggleUrlField() {
            ...
            const urlInput = document.getElementById('url_menu_input');
            if (!parentId) {
