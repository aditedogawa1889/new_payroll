<x-admin-layout>
    <x-slot name="header">
        <div class="row flex items-center justify-between">
            <div class="col-sm-6">
                <h1 class="m-0 text-2xl font-bold text-gray-800">Edit Role</h1>
            </div>
        ...
        <div class="admin-card">
            <form action="{{ route('roles.update', $role->id) }}" method="POST" class="p-6">
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="name" value="{{ $role->name }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-adminlte-primary focus:border-adminlte-primary" required>
                </div>
                        ...
                        @foreach($menus as $menu)
                            <div class="border rounded-lg p-4 bg-gray-50/50">
                                <label class="flex items-center font-bold text-gray-800 cursor-pointer">
                                    @if($menu->permission)
                                        <input type="checkbox" name="permissions[]" value="{{ $menu->permission->name }}" {{ in_array($menu->permission->name, $rolePermissions) ? 'checked' : '' }} class="mr-3 rounded border-gray-300 text-adminlte-primary focus:ring-adminlte-primary">
                                    @endif
                                        ...
                                        @foreach($menu->submenus as $submenu)
                                            @if($submenu->permission)
                                                <label class="flex items-center text-sm text-gray-600 hover:text-gray-900 cursor-pointer transition-colors">
                                                    <input type="checkbox" name="permissions[]" value="{{ $submenu->permission->name }}" {{ in_array($submenu->permission->name, $rolePermissions) ? 'checked' : '' }} class="mr-3 rounded border-gray-300 text-adminlte-primary focus:ring-adminlte-primary">
